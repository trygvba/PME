...
%				ABSTRACT:
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\abstract{This project is an exposition of the General Porous Medium Equation, which contains both the porous medium equation and the heat equation as special cases as well as the Stefan problem when considering the degenerate case. The first part is devoted to presenting some theory on classical solution with attention paid to important estimates. 

 The theory is then extended to a weak setting for the weakly degenerate case. Again, proving well posedness will be the main focus, and to that end the concept of weak energy solutions is presented. In this framework existence, uniqueness and stability is proved, and attention is paid to showing that the estimates for classical solutions are inherited by weak solutions. }
...
\begin{enumerate}[i)]
	Suppose $\Omega$ is connected, and $u\in C^2_1(Q_T)\cap C(\overline{Q}_T)$, and $\partial_t + L$ is uniformly parabolic, then:
	\item If
	\begin{equation*}
...
\label{lady_parabolic_eq}
	u_t - \sum_{i=1}^d\frac{\partial}{\partial x_i}a_i(x,t,u,\nabla u) + a(x,t,u,\nabla u) = 0
\end{equation}
...
Assume the following conditions are satisfied:
	\begin{enumerate}[a)]
		...
		\item $u_0\in C^2(\Omega)$, and $u_0$, $\nabla u_0$ and $\Delta u_0$ vanish at the boundary.
...
\setcounter{obs}{0}
\begin{obs}
If $f\in L^2(Q_T)$ we can do the same trick on the right hand side as we did in deriving (\ref{spatial_derivative_bound_w_poaincare}), to get that $\Phi(u) \in L^2(0,T: H^1_0(\Omega))$.
\end{obs}
...
Notice that by Young's inequality $||f_n||_{L^2(Q_T)} \leq ||\tilde{f}_n||_{L^2(Q_T)} \leq ||f||_{L^2(Q_T)}$. Because of $\partial_t (\tilde{f}_n* \varphi_{\frac{1}{n}}) = (\partial_t \tilde{f}_n) *\varphi_{\frac{1}{n}}$, we also have that $||\partial_t f_n||_\infty \leq ||\partial_t f||_\infty$. This is true for any derivative of $f$ that exists, but it's only the time derivative estimate here that will become useful to us.
...
the sequences $(w_n)$ and $(\nabla w_n)$ are uniformly bounded in $L^2(Q_T)$. In other words $(w_n)$ is uniformly bounded $L^2(0,T: H^1_0(\Omega))$. Moreover, there are constants $B_1, B_2 >0$ depending only on $M$ from Lemma \ref{prop:classical_sols_seq} and $\Phi$, so that
\begin{equation}
\label{gradwn_bound_uniform}
||\nabla w_n||_{L^2(Q_T)} \leq B_1||u_0||_{L^1(\Omega)} + B_2||f||_{L^2(Q_T)}.
\end{equation}
for all $n$.
\end{lemma}
\begin{proof}
Since $u_n$ are classical solutions, we use (\ref{spatial_derivative_bound_w_poaincare}) to get that
	\begin{equation}
	...
	\begin{align*}
	\int_\Omega \Psi_n(u_{0,n})dx &\leq \frac{B_1}{2} ||u_{0,n}||_{L^1(\Omega)} \\
	&\leq \frac{B_1}{2} ||u_0||_{L^1(\Omega)}
	\end{align*}
	...
	\end{equation*}
	$w_n$ is also uniformly bounded in $L^2(Q_T)$. Lastly, setting $B_2 = 2C$ and multplying (\ref{exist_proof_spatial}) by $2$ we get
	\begin{equation}
	\iint_{Q_T} |\nabla w_n|^2 dxdt \leq B_1 ||u_0||_{L^1(\Omega)} + B_2 ||f||_{L^2(Q_T)}.
	\end{equation}
\end{proof}
...
\begin{description}
	...
	\end{align*}
	 We have that $w_{n_j}$ is a bounded sequence, seeing as $u_{n_j}$ is bounded. Hence we can use Lemma \ref{prop:inverse_uniform} to make the first term in the above inequality arbitrarily small. The second term is controlled by the almost everywhere convergence of $w_{n_j}$ to $w$ and the continuity of $\Phi^{-1}$. That means that
	 \begin{equation*}
	 u_{n_j} \to u = \Phi^{-1}(w)
	 \end{equation*}
	...
	For almost every $t \in [0,T]$ we have by Fatou's Lemma \citep[p. 162]{weiss1999course} that since $|u_{n_j}(t)|$ converges almost everywhere to $|u(t)|$ and are nonnegative, then
	\begin{equation*}
	||u(t)||_{L^1(\Omega)} \leq \liminf_{j \to \infty} ||u_{n_j}(t)||_{L^1(\Omega)}.
	\end{equation*}
	...
	\begin{align}
	||u(t)||_{L^1(\Omega)} &\leq \liminf_{j \to \infty} \left( ||u_{0,n_j}||_{L^1(\Omega)} + ||f_{n_j}||_{L^1(Q_T)}\right) \nonumber \\
	...
	\begin{equation}
	|\Psi_{n_j}(u_{n_j}) - \Psi(u)| \leq |\Psi_{n_j}(u_{n_j}) - \Psi(u_{n_j})| + |\Psi(u_{n_j}) + \Psi(u)|.
	\end{equation}
	Because $|u_n| \leq M$ we use that $\Psi_n$ converges uniformly on compact sets (see Lemma \ref{prop:Psi_uniform_convergence}) to control the first term. The second term is controlled by the almost everywhere convergence of $u_{n_j}$ to $u$ and the continuity of $\Psi$.
	...
	\begin{align*}
	\int_\Omega \Psi(u(t))dx &\leq \liminf_{j \to \infty} \left( \int_\Omega \Psi_{n_j}(u_{0,n_j})dx + C\iint_{Q_T}f_{n_j}^2 dxdt \right)\nonumber \\
...
\iint_{Q_T^\tau} |\partial_t z_n|^2 dxdt \leq \frac{||\partial_t \zeta||_\infty}{2}\left( B_1||u_{0,n}||_{L^1(K_n)} + B_2||f_n||_{L^2(Q_n)}\right) + \iint_{Q_n}|\partial_t (f_n \zeta ) w_n| dxdt,
\end{equation}
...
\end{align*}
By Lemma \ref{prop:Phi_uniform_convergence} we can take find an $N \in \mathbb{N}$ so that
\begin{equation*}
...
In the remainder of this section we'll assume that $\Phi$ is continuous and strictly increasing, and that $\Phi( \pm \infty) = \pm \infty$, i.e. satisfies assumption \ref{ass:Phi_general}. Then it is obvious that both $\Phi$ is both one-to-one and onto, and so has an inverse $\Phi^{-1}$.

\begin{lemma}
\label{lem:Phi_inv_cont}
With assumption \ref{ass:Phi_general} $\Phi^{-1}$ is strictly increasing, and continuous.
\end{lemma}
\begin{proof}
...
\begin{lemma}
With the same assumptions as Lemma \ref{lem:Phi_inv_cont} $\Phi_n^{-1}$ converges pointwise to $\Phi^{-1}$.
\end{lemma}
...
\end{equation*}
which by the strict monotonicity of $\Phi_n$ means that $x_n \leq x_2$. A similar argument yields $x_n \geq x_1$, and so we have that for every $n \geq N$ that $x_n \in [x_1, x_2]$. This means that $x_n$ is a bounded sequence, and hence we can use the uniform convergence of $\Phi_n$ on compact sets to conclude that $y_n$ converges to $y$. Finally we have that
\begin{equation*}
...
With the same assumptions as Lemma \ref{lem:Phi_inv_cont} $\Phi_n^{-1}$ converges to $\Phi^{-1}$ uniformly on compact sets.
\end{lemma}
...
Let $f: \mathbb{R} \to \mathbb{R}$ be a strictly monotone, continuous function, and let $\{f_n\}$ be a family of strictly monotone, continuous functions that converge to $f$ uniformly on compact sets. All functions are so that $f(\pm \infty) = \pm \infty$. Then $f_n^{-1}$ converges to $f^{-1}$ uniformly on compact sets.
\end{lemma}

\begin{proof}
The proof is identical to that of Lemma \ref{prop:inverse_uniform} with only minor alterations in notation.
\end{proof}
...
\end{equation*}
We have of course complete control of the latter of these terms by making $n$ sufficiently large. For the first term we use part iv) of theorem 7 of appendix C in \citep{evans}, to state that $\Phi'*\varphi_{\frac{1}{n}}$ converges to $\Phi$ in $L^1_{loc}(\mathbb{R})$. With this we deduce that $Z_n$ converges pointwise almost everywhere to $Z$ on any compact set, at least along a subsequence. Since all $Z_n$ and $Z$ are continuous the convergence happens everywhere on any compact set.
\end{proof}
...
With the same assumptions as Lemma \ref{lem:Phi_inv_cont} $Z_n$ converges to $Z$ uniformly on compact sets.
\end{lemma}
\begin{proof}
...
With the same assumptions as Lemma \ref{lem:Phi_inv_cont}, $Z$ is strictly increasing and continuous, and thus permits a strictly increasing and continuous inverse. Furthermore $Z_n^{-1}$ converges to $Z^{-1}$ uniformly on compact sets.
\end{lemma}
\begin{proof}
...
We have from Lemma \ref{prop:Z_uniform_convergence} that $Z_n$ converges uniformly to $Z$ on compact sets, and so we can simply use Lemma \ref{lem:Inverse_uniform_general} to conclude that $Z_n^{-1}$ converges uniformly to $Z^{-1}$ on compact sets.
\end{proof}
