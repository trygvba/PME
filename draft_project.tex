...
\begin{description}
	...
	\end{align*}	 
	pops out. Restricting ourselves to only $\eta \in C^\infty_c(Q_T)$, and doing integration by parts on the term involving $w$, yields
	\begin{equation*}
	\bigg|\iint_{Q_T}(u_{n_j} - u_{n_k})\partial_t \eta dxdt \bigg| \leq \bigg|\iint_{Q_T} (w_{n_j} - w_{n_k})\Delta\eta dxdt \bigg| +  \bigg|\iint_{Q_T}(f_{n_j} - f_{n_k})\eta dxdt\bigg|,
	\end{equation*}
	where the term concerning the initial data has dropped out following that $\eta \big|_{t=0} = 0$. The first and the last term can both be made arbitrarily small by the convergence of $w_{n_j}$ and $f_n$ respectively. And so $\partial_t u_{n_j}$ in the weak sense is a Cauchy sequence in $L^1_{loc}(Q_T)$, and since $Q_T$ is bounded, also in $L^1(Q_T)$.
